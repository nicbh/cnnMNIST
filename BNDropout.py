...
h_conv3 = tf.matmul(h_pool2_flat, W_fc2) + b_fc2

keep_prob = tf.placeholder(tf.float32)
y_conv = tf.nn.dropout(h_conv3, keep_prob)
...
for epochs in range(20):
    for i in range(200):
        ...
        train_label = batch[1]
        if i % 1000 == 0:
            train_accuracy = accuracy.eval(feed_dict={x: train_data, y_: train_label, phase_train: False, keep_prob: 1})
            print("epochs %d, step %d, training accuracy %g" % (epochs, i, train_accuracy))
        train_step.run(feed_dict={x: train_data, y_: train_label, phase_train: True, keep_prob: 0.5})
...
for i in range(10):
    ...
    test_data = batch[0]
    test_label = batch[1]
    acc += accuracy.eval(feed_dict={x: test_data, y_: test_label, phase_train: False, keep_prob: 1})
acc = acc / 10
